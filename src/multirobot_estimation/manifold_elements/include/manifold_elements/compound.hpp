                             ...
                             templateSum(Args::tangent_dim_...)>
{
 ...
 public:
  ...
  template <size_t N>
  using ElementType = typename TypeList<Args...>::template type<N>::ElementType;
  ...
  template <unsigned int N>
  const typename TypeList<Args...>::template type<N> &get() const
  {
    return std::get<N>(t_);
  }

  template <unsigned int N>
  typename TypeList<Args...>::template type<N> &get()
  {
    return std::get<N>(t_);
  }
...
#else
  CompoundElement &operator+=(const TangentVec &diff) override
  {
    add(diff, *this);
    return *this;
  }
